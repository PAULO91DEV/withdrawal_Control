...
import 'package:controleRetirada/screens/itemsproduct/items_products_screen.dart';
import 'package:controleRetirada/screens/products/products_controller.dart';
...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  void initState() {
    ...
    this._controller = ProductsController(
  ...
  Widget build(BuildContext context) {
    ...
    ConfirmEntity confirmRequestEntity;
    return WillPopScope(
      onWillPop: () async {
  ...
  List<Widget> createItems() {
    ...
    this._controller.entitie?.forEach((element) {
      widgets.add(
        createItem(
          element.qtyRemaining,
  ...
  Container createItem(
    ...
    return Container(
      ...
      child: TextButton(
        onPressed: () {
          //          if (productEntity.codeReturn == 0) {
          this._controller.addItemToCart(productEntity, (
            error,
            isFinishedItemsCart,
            isGoNextLevel,
            returnDescription,
            ...
            if (isGoNextLevel) {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => ItemsProductScreen(
                          listCartProduct: this._controller.listCart,
                          employeeEntity: this.widget.employeeEntity,
                          eventEntity: this.widget.eventEntity,
                          productEntity: productEntity,
                        )),
              ).then((_) {
                this._controller.load();
                  ...
                  child: Container(
                    child: Column(
                      children: [
                        ...
                        textSaldo(textQtd, productEntity),
                      ],
                    ),
                  ),
                ),
  ...
  Text textSaldo(int textQtd, ProductEntity productEntity) {
    if (productEntity.existsItem == 0) {
      return Text(
        "Saldo : $textQtd",
        overflow: TextOverflow.ellipsis,
        maxLines: 2,
